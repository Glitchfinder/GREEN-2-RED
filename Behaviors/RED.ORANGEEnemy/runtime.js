...
/////////////////////////////////////
// Behavior class
cr.behaviors.REDORANGEEnemy = function(runtime)
{
...
(function ()
{
	var behaviorProto = cr.behaviors.REDORANGEEnemy.prototype;
	...
	behaviorProto.Instance = function(type, inst)
	{
		...
		this.currentDirection = 0; // Current direction: 0 = left, 1 = right, 2 = up, 3 = down;
	};
	...
	behinstProto.checkCharge = function (location)
	{
		if(this.charging)
			return false;
		...
		{
		case 0:
			var tempValue = this.inst.x
			
			while(this.inst.x > location + 19)
			{
				this.inst.x -= 32;
				this.inst.set_bbox_changed();
		...
		case 1:
			...
			break;
		case 2:
			var tempValue = this.inst.y
			
			while(this.inst.y > location + 19)
			{
				this.inst.y -= 32;
				this.inst.set_bbox_changed();
	...
	behinstProto.tick = function ()
	{
		...
		else
		{
			...
			for(var i = 0; i < this.players.players.length; i += 1)
			{
				...
				if (!this.charging)
				{
					var playerX = this.players.players[i].instances[0].x;
					var playerY = this.players.players[i].instances[0].y;
					
					var tempX = this.inst.x;
					var tempY = this.inst.y;
					
					switch(this.currentDirection)
					{
					case 0:
						if(playerY > (tempY - 10) && playerY < (tempY + 10) && playerX < tempX)
						{
							this.inst.opacity = 0;
							this.checkCharge(playerX);
						}
						break;
					case 1:
						if(playerY > (tempY - 10) && playerY < (tempY + 10) && playerX > tempX)
						...
						break;
					case 2:
						if(playerX > (tempX - 10) && playerX < (tempX + 10) && playerY < tempY)
						{
							this.inst.opacity = 0;
							this.checkCharge(playerY);
						}
						break;
					default:
						if(playerX > (tempX - 10) && playerX < (tempX + 10) && playerY > tempY)
